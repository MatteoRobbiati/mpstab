...
def main():
    ...
    all_zero = '0'*N
    all_plus = '+'*N
    random_mix = ''.join([possible_symbols[np.random.randint(len(possible_symbols))] for _ in range(N)])
    ...
    for be in basis_elements:
        ...
        contract_ghz(ghz, N)
        print(f'Network contraction completed. <{be}|GHZ>: {ghz.tensornet.nodes["F"]["tensor"].item()}')
    ...
    print(f'Contracted, random tensor network: {star.tensornet.nodes["F"]["tensor"].item()}')
