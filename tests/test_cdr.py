...
plt.vlines(0, min(training_data["noise-free"]) - delta, max(training_data["noise-free"]) + delta, color="black", ls="-", lw=1)
plt.hlines(0, min(training_data["noisy"]) -  delta, max(training_data["noisy"]) + delta, color="black", ls="-", lw=1)
plt.grid(True)
plt.legend()
