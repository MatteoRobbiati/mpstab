...
class MPO(TensorNetwork):
    ...
    def __init__(
        ...
        assert n_tensors == len(
            physical_directions
        ), f"Mismatch in the number of tensors and physical legs, {n_tensors}!={len(physical_directions)}."
        assert (
            n_tensors == len(link_directions) + 1
        ), f"Mismatch in the number of tensors ({n_tensors}) and link directions ({len(link_directions)}, should be {n_tensors-1})."
