...
from .tableaus import Tableau, CNOT, SWAP, H, S, X, Z
...
def clear_ZY(to_be_cleared:Pauli, to_be_updated:Pauli, shift:int=0)->Generator[Tableau, None, None]:
    """
...
def first_X(p:Pauli)->int:
    for i in range(p.n):
        if p._has_X(i): return i
    raise ValueError(f'The string {p} has no X.')

def reduce_Xs(to_be_reduced:Pauli, to_be_updated:Pauli, shift:int=0)->Generator[Tableau, None, None]:
    """
...
def layer(tableau_x:Pauli, tableau_z:Pauli, shift:int=0)->Generator[Tableau, None, None]:
    """
