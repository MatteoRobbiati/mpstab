...
def draw_tn(tn, show_labels=False, title=""):
    ...
    for idx, group in enumerate(group_keys):
        ...
        if p > 0:
            ...
            peripheral_angles = list(all_angles[:left_count]) + list(
                all_angles[-right_count:]
            )
    ...
    nx.draw_networkx_nodes(
        tn.tensornet, pos, node_size=650, node_color=node_colors, ax=ax
    )
    nx.draw_networkx_labels(tn.tensornet, pos, font_size=10, font_weight="bold", ax=ax)
    ...
    legend_patches = [
        Patch(facecolor=group_color_map[group], label=group) for group in group_keys
    ]
...
def _bond_dimension_cut(U, D, V, max_bond_dimension):
    ...
    return (
        U[:, :bond_dimension],
        D[:bond_dimension] / np.sqrt(np.sum(D[:bond_dimension] ** 2)),
        V[:bond_dimension, :],
    )
